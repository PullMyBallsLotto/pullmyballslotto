...
UI theme is bold Black/Red/White. Includes paywall tiers: Free 5 runs, 20‑pack, Monthly Unlimited.
"""
...
# --- Paywall tiers (free 5 runs, 20-run pack, unlimited monthly) ---
...
def verify_gumroad_tier(license_key: str) -> str:
    ...
    We identify the tier by which product_id the license validates against.
    """
    if not license_key:
        return "none"
    try:
...
def _inject_global_css():  # pragma: no cover - UI only
    ...
    st.markdown(
        ...
        .pill { display:inline-block; padding:4px 10px; border-radius:999px; font-size:.8rem; background:rgba(229,9,20,.15); color:#fff; border:1px solid rgba(229,9,20,.35); font-weight:700; }
        ...
        .chip-btn { background: #1a1a1a; color:#fff; border:1px solid rgba(255,255,255,.08); border-radius:999px; padding:6px 0; font-weight:800; text-align:center; }
        .chip-btn.sel { background: var(--accent); border-color: rgba(229,9,20,.7); }
        </style>
        """,
...
# ---------------------- Legal / Terms ----------------------
TERMS_VERSION = "2025-08-25"
NO_REFUNDS_TEXT = "All sales are final and non-refundable except where required by applicable law. Chargebacks may result in license revocation."


def legal_markdown() -> str:
    return f(
        """
**Educational & Entertainment Only.** This app analyzes public lottery data and simulates outcomes. It does **not** predict results, increase odds, or offer gambling services. Use is at your own risk.

**No Affiliation.** Not affiliated with any lottery commission, Powerball®, MUSL, or state lotteries. Trademarks belong to their owners.

**No Financial / Legal Advice.** Content is informational only and not professional advice. You are solely responsible for your decisions.

**Compliance & Age.** You must comply with your local laws and be of legal age to buy lottery tickets where you live.

**Data Sources & Accuracy.** Data is aggregated from public/official pages on a best‑effort basis and may be incomplete, delayed, or inaccurate. The app may fall back to alternate sources or cached data. No warranty of correctness.

**Purchases & Access.** License keys are validated via Gumroad. Keys may be disabled for abuse, sharing, chargebacks, or fraud.

**Refunds.** {NO_REFUNDS_TEXT}

**Limitation of Liability.** To the maximum extent permitted by law, the app and authors are not liable for any indirect, incidental, consequential, special, exemplary, or punitive damages, or any loss of profits, revenue, data, or goodwill, arising from or related to your use of the app.

**Indemnity.** You agree to defend, indemnify, and hold harmless the authors from any claims, damages, liabilities, and expenses arising from your use of the app or violation of these terms.

**Updates.** Terms may be updated; continued use constitutes acceptance. Terms version: {TERMS_VERSION}.
        """
    )
...
def main() -> None:  # pragma: no cover - UI only
    ...
    # Terms / Legal (must accept to run simulations)
    with st.expander("Legal & Terms of Use", expanded=False):
        st.markdown(legal_markdown())
        st.info(NO_REFUNDS_TEXT)
    agree_terms = st.checkbox("I agree to the Terms of Use (including No‑Refund policy)", value=False)
    ...
    with colL:
        st.subheader("Access")
        # Initialize session credits/state
        ...
        with st.expander("Buy credits/plans"):
            ...
            st.markdown(f"> **Refunds:** {NO_REFUNDS_TEXT}")

        licensed = is_paid  # keep variable name used later
    ...
    # Loud CTA buttons after data load
    cta1, cta2, cta3 = st.columns([2,1,1])
    with cta2:
        if st.button("🎲 Quick Pick 10 (Hot) & Run", use_container_width=True):
            try:
                w_arr, r_arr = make_weight_arrays(df_pb, white_max=69, red_max=26, smoothing=1.0, cold=False, recency_n=0)
                rng_qp = np.random.default_rng(None)
                new = []
                for _ in range(10):
                    ws, rr = _generate_pick_weighted(rng_qp, 69, 26, w_arr, r_arr)
                    new.append((ws, rr))
                ...
                st.success("Added 10 quick picks and queued a run.")
            except Exception as e:
                st.warning(f"Quick pick failed: {e}")
    with cta3:
        st.button("🎛️ Open Simulator", use_container_width=True)

    tab_stats, tab_sim = st.tabs(["Stats (PB)", "Simulator"])
    ...
    with tab_sim:
        ...
        for n in range(1, 70):
            ...
            with c:
                sel = n in current_w
                label = f"{'✓ ' if sel else ''}{n}"
                if st.button(label, key=f"chip_w_{n}"):
                    if sel:
                        current_w.remove(n)
                    else:
                        if len(current_w) < 5:
                            current_w.add(n)
        st.session_state["chip_whites"] = sorted(current_w)
        st.caption(f"Selected whites: {st.session_state['chip_whites']}")

        # Red chips
        st.markdown("<div class='section-label'>Select 1 Powerball</div>", unsafe_allow_html=True)
        cols_r = st.columns(10)
        cur_r = st.session_state["chip_red"]
        for n in range(1, 27):
            c = cols_r[(n-1) % 10]
            with c:
                sel = (cur_r == n)
                label = f"{'✓ ' if sel else ''}{n}"
                if st.button(label, key=f"chip_r_{n}"):
                    st.session_state["chip_red"] = n if not sel else None
        st.caption(f"Selected red: {st.session_state['chip_red']}")

        cc1, cc2, cc3 = st.columns([1,1,2])
        with cc1:
            if st.button("Add pick from chips"):
                if len(st.session_state["chip_whites"]) == 5 and st.session_state["chip_red"]:
                    pick = (sorted(st.session_state["chip_whites"]), int(st.session_state["chip_red"]))
                    if pick not in st.session_state["built_picks"]:
                        st.session_state["built_picks"].append(pick)
        with cc2:
            if st.button("Clear chips"):
                st.session_state["chip_whites"] = []
                st.session_state["chip_red"] = None

        # Classic inputs remain (for keyboard users)
        ...
        # Quick Picks panel (adds to your picks)
        st.markdown("**Quick Picks (add to your picks)**")
        qp_col1, qp_col2, qp_col3 = st.columns([1,1,2])
        with qp_col1:
            n_qp = st.selectbox("How many?", [1,5,10,20], index=1)
        with qp_col2:
            qp_mode = st.radio("Mode", ["Uniform", "Historical (hot)", "Historical (cold)"], index=1)
        with qp_col3:
            qp_recency = 0
            if not qp_mode.startswith("Uniform"):
                qp_recency = st.slider(
                    "Recency window (last N draws; 0 = all)",
                    min_value=0,
                    max_value=int(len(df_pb)) if isinstance(df_pb, pd.DataFrame) else 0,
                    value=0,
                    step=10 if len(df_pb) > 200 else 1,
                )

        if st.button("Generate & Add Quick Picks"):
            rng_qp = np.random.default_rng(None)
            if qp_mode.startswith("Uniform"):
                w_arr = r_arr = None
            else:
                w_arr, r_arr = make_weight_arrays(
                    df_pb, white_max=69, red_max=26, smoothing=1.0,
                    cold=("cold" in qp_mode.lower()),
                    recency_n=qp_recency,
                )
            new = []
            for _ in range(int(n_qp)):
                if w_arr is not None:
                    ws, rr = _generate_pick_weighted(rng_qp, 69, 26, w_arr, r_arr)
                else:
                    ws, rr = _generate_pick_uniform(rng_qp, 69, 26)
                new.append((ws, rr))
            for pick in new:
                if pick not in st.session_state["built_picks"]:
                    st.session_state["built_picks"].append(pick)
            st.success(f"Added {len(new)} quick picks.")
            render_ticket_grid(new, title_prefix="Quick")
        ...
        # (Removed) text picks per request
        picks_text = []

        # Unified picks (built only, deduped)
        unified: List[Tuple[List[int], int]] = []
        seen = set()
        for whites, r in st.session_state["built_picks"]:
        ...
        if st.session_state.get("do_run"):
            ...
            if not st.session_state.get("terms_agreed", False):
                st.error("Please accept the Terms of Use (including No‑Refund policy) before running simulations.")
                st.stop()
            ...
            if is_unlimited:
                allowed = True
            elif has_pack and st.session_state.get("pack_runs_left", 0) > 0:
                allowed = True
                st.session_state["pack_runs_left"] -= 1
                st.success(f"Pack run used. Runs left: {st.session_state['pack_runs_left']}")
            elif st.session_state.get("free_runs_left", 0) > 0:
                allowed = True
                st.session_state["free_runs_left"] -= 1
                st.info(f"Free run used. Free runs left: {st.session_state['free_runs_left']}")

            if not allowed:
                st.error("No runs left. Activate a license key or buy a pack to continue.")
                st.stop()
            ...
            if not unified and not (use_auto_pick):
                ...
                try:
                    ...
                    st.caption("Use at your own risk. No affiliation. No refunds except where required by law.")
...
Adds:
- Power Play + EV simulation (uniform/weighted/fixed multipliers, 10x cap aware)
- Weighted Auto-pick (hot/cold; optional recency window)
- Pick Builder UI + CSV import/export
- Latest draw detail (best-effort), jackpot estimate (best-effort)
- Terms / legal footer; first-visit About/Disclaimer notice
- Separate Terms tab (plus a standalone 'pages/Terms.py' in a multipage setup)
- Downloadable Terms (.md, .html)
- Pyodide/tzdata-safe ZoneInfo bootstrap

Not affiliated with any lottery commission. Educational/entertainment only.
"""

from __future__ import annotations

# Allow `import app` in multipage mode (pages/Terms.py imports this)
import sys as _app_sys
_app_sys.modules.setdefault("app", _app_sys.modules[__name__])
...
# Optional BeautifulSoup (HTML parser)
try:  # pragma: no cover
    from bs4 import BeautifulSoup  # type: ignore
    BS4_AVAILABLE = True
except Exception:
...
# Streamlit (optional at import-time for CLI safety)
try:  # pragma: no cover
    import streamlit as st
    from streamlit_autorefresh import st_autorefresh
except Exception:  # pragma: no cover
...
# ---------------------- tzdata bootstrap for Pyodide/minimal envs ----------------------
def _ensure_tzdata_loaded_for_pyodide() -> None:
    """Load tzdata under Pyodide so ZoneInfo works (best-effort)."""
    try:
        import tzdata  # noqa: F401
        return
    except Exception:
...
# ---------------------- Legal/Terms content ----------------------
TERMS_VERSION = "2025-08-25"
NO_REFUNDS_TEXT = "All purchases are **non-refundable** except where required by law."
...
def _terms_download_payloads() -> Tuple[bytes, bytes, str]:
    md = legal_markdown()
    html = (
        "<!doctype html><html><head><meta charset='utf-8'>"
        "<title>PullMyBallsLotto — Terms</title>"
        "<style>body{font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;"
        "max-width:720px;margin:40px auto;line-height:1.55;color:#222}"
        "h1,h2{margin:0 0 .5rem} pre{white-space:pre-wrap}</style>"
        "</head><body><h1>Terms of Use</h1><pre>" + _escape_html(md) + "</pre></body></html>"
    )
    fname = f"pullmyballslotto_terms_{TERMS_VERSION}"
    return md.encode("utf-8"), html.encode("utf-8"), fname
...
def prize_for_result(match_white: int, pb_matched: bool, jackpot_estimate: Optional[int]) -> int:
    val = POWERBALL_PRIZES.get((match_white, pb_matched), 0)
    return int(jackpot_estimate or 0) if val == "JACKPOT" else int(val)
...
def load_powerball() -> pd.DataFrame:
    rows: List[Dict[str, int | datetime.date]] = []
    # Texas CSV (schema sometimes changes)
    try:
        tx = pd.read_csv(TX_PB_CSV, dtype=str, engine="python", on_bad_lines="skip")
        date_col = None
        num_col = None
...
def make_weight_arrays(
    ...
    *,
    ...
    r = r / (r.sum() if r.sum() > 0 else 1.0)
    if not np.isfinite(w).all(): w = np.full(white_max, 1.0 / white_max)
    if not np.isfinite(r).all(): r = np.full(red_max, 1.0 / red_max)
    return w, r
...
POWER_PLAY_10X_CAP = 150_000_000  # 10× allowed only when jackpot ≤ cap
PP_WEIGHTS_WITH_10X = {2: 0.557, 3: 0.303, 4: 0.070, 5: 0.047, 10: 0.023}
PP_WEIGHTS_NO_10X   = {2: 0.571, 3: 0.309, 4: 0.069, 5: 0.051}
    ...
    for _ in range(draws):
        ...
        if power_play:
            draw_multiplier = _draw_power_play_multiplier(
                ...
                fixed=pp_fixed_multiplier,
                weights_with_10x=PP_WEIGHTS_WITH_10X,
                weights_no_10X=PP_WEIGHTS_NO_10X,  # type: ignore[arg-type]
            ) if 'PP_WEIGHTS_NO_10X' in globals() else _draw_power_play_multiplier(
                rng, allow_10x=eff_allow_10x, mode=eff_mode, fixed=pp_fixed_multiplier,
                weights_with_10x=PP_WEIGHTS_WITH_10X, weights_no_10x=PP_WEIGHTS_NO_10X
            )
        else:
            draw_multiplier = 1
...
def main() -> None:  # pragma: no cover
    ...
    # First-visit About/Disclaimer (dismiss to hide)
    if "about_seen" not in st.session_state:
        st.session_state["about_seen"] = False
    if not st.session_state["about_seen"]:
        with st.container():
            st.info("**About this app** — Educational tool; not predictions; not affiliated. Read the Terms tab.")
            c1, c2 = st.columns(2)
            if c1.button("Got it", key="about_ok"):
                st.session_state["about_seen"] = True
                st.rerun()
            c2.caption("See the **Terms** tab or the standalone **Terms** page.")
    ...
    tab_overview, tab_stats, tab_sim, tab_terms = st.tabs(["Overview", "Stats", "Simulator", "Terms"])
        ...
        if st.button("Run simulation", type="primary"):
            if not unified and not use_auto_pick:
                ...
                try:
                    ...
                    if licensed:
                        tiers_df, metrics_df = simulation_to_csvs(
                            res,
                            draws=res["draws"],
                            num_picks=(auto_pick_n if use_auto_pick and auto_each_draw else max(len(unified), auto_pick_n)),
                        )
                        c1, c2 = st.columns(2)
                        with c1:
                            st.download_button(
    ...
    # Terms tab (print/download friendly inside main app)
    with tab_terms:
    ...
    st.markdown(
        """
        <div style="position:sticky;bottom:0;left:0;right:0;padding:8px 12px;background:#0f1116;border-top:1px solid rgba(255,255,255,.08);color:#c7c9cc;font-size:12px;">
        <b>Legal:</b> Educational only. Not affiliated. {no_refunds}<br/>
        <span>© {year} PullMyBallsLotto. Terms version {ver}. See the Terms tab or the Terms page.</span>
        </div>
        """.format(no_refunds=NO_REFUNDS_TEXT, year=datetime.now().year, ver=TERMS_VERSION),
        unsafe_allow_html=True,
    )
