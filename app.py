...
def test_make_weight_arrays_hot_vs_cold():
    df = pd.DataFrame({
        'W': [[1,2,3,4,5],[1,2,3,4,6],[1,2,3,7,8]],
        'R': [6,7,8],
        'date': pd.to_datetime(['2025-01-01','2025-01-02','2025-01-03']).date,
    })
    df['draw_date'] = df['date']
    w_hot, r_hot = app.make_weight_arrays(df, white_max=10, red_max=10, smoothing=1.0, cold=False, recency_n=0)
    w_cold, r_cold = app.make_weight_arrays(df, white_max=10, red_max=10, smoothing=1.0, cold=True, recency_n=0)
    assert w_hot[0] > w_hot[9]
    assert w_cold[0] < w_cold[9]


def test_make_weight_arrays_recency():
    df = pd.DataFrame({
    ...
    df['draw_date'] = df['date']
    w_all, r_all = app.make_weight_arrays(df, white_max=10, red_max=10, recency_n=0)
    w_recent, r_recent = app.make_weight_arrays(df, white_max=10, red_max=10, recency_n=1)
    assert w_recent[5] > w_all[5]
    assert r_recent[0] > r_all[0]


def test_make_weight_arrays_normalize():
    df = pd.DataFrame({"W": [], "R": []})
    w, r = app.make_weight_arrays(df, white_max=5, red_max=3, smoothing=1.0)
